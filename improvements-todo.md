...
Last updated: 2026-08-17 01:17 America/Chicago
...
- [ ] **IN PROGRESS — Require supervised diagnosis at round 3.** Current operating rule: no third blind implementation/gate cycle. Route the exact failing artifact to a focused repair worker, prove the failure or invalidate it, then permit one supervised gate. JOR-206, JOR-214, and JOR-218 have now completed this recovery path and merged. JOR-251's `response-contract` repair is pushed, reconciled, focused-green 38/38, and protected by a lease while the cap-release seam below is added. JOR-283 is leased for a real clean-checkout fixture defect discovered during its state audit. Machine-enforced round-three routing itself is still TODO.

- [ ] **IN PROGRESS — Complete a valid supervised repair without falsifying history.** JOR-251 exposed a missing tracker action after the rejection cap: `verdict-reset` truthfully means an invalid gate, while `rescope` truthfully means different work; neither means valid failures were fixed under supervision on a new HEAD. Add a human-only, reason-bearing, provider-neutral marker that retires prior gate rejection history only, preserves merge history, and lets the repaired ticket return directly to review. Snapshot/planner derivation, automated-caller refusal, public-seam holding coverage, and a planted mutant are in progress.
...
- [ ] **IN PROGRESS — Prioritize closure and dependency impact.** Current recovery order favors a nearly-green ticket or a ticket that unlocks dependents over queue age alone. JOR-206, JOR-214, and JOR-218 have merged; Loom immediately chained into fresh implementation lanes for JOR-231, JOR-233, and JOR-236. JOR-240 was narrowed to its coherent E6-only wiring surface, its hold was released, and it waits behind the three-primary-lane cap. JOR-251's reconciled repair at `0d8206b` remains the largest fan-out and will receive one supervised API gate as soon as the honest cap-release marker lands. JOR-283's hidden open PR and clean-checkout defect are under supervised repair. Planner scoring and holding tests remain TODO.
