...
Last updated: 2026-08-16 23:25 America/Chicago
...
- [ ] **IN PROGRESS — Add a supervised-repair lease.** A ticket under supervisor/sub-agent repair needs a machine-readable lease. Ordinary waves must not launch a duplicate implementer or gate until the lease is released or expires. The isolated TDD test is red at 2 pass/6 fail and independently demonstrates the missing CLI, snapshot/plan representation, planner deferral, and spawn-boundary guard, including a real duplicate `impl-10` launch. Implementation is underway without changing live build state. This closes the duplicate JOR-214/JOR-287 collisions and the 23:14 JOR-286 race, where a heartbeat launched `impl-286` during supervised reconciliation; it was stopped before editing the verified worktree.

- [ ] **IN PROGRESS — Prioritize closure and dependency impact.** Current recovery order favors a nearly-green ticket or a ticket that unlocks dependents over queue age alone. JOR-208 is merged and closed. JOR-287 passed review and is in its serialized UI merge preflight to unlock JOR-218. JOR-286 passed its API gate and review to unlock JOR-244, but its merge handoff collided with JOR-287's merge lock and requires a supervised requeue. JOR-206 is repaired and reconciled, and still unlocks JOR-230/JOR-212 after its UI turn. Planner scoring and holding tests remain TODO.
