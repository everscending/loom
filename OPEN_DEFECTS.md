...
**2026-08-15**; D-SNAP-18 was filed and closed the same day, out of P96's own review of
the code it left behind. Severity is a same-pass judgment
...
`snapshot.jq` `blocked_report_of` attached to every ticket whose thread carried an `orch-blocked`
trailer, whatever the ticket's current state, and computed `released` from one signal only: an
`orch-unblock` trailer newer than that block. `lane.sh` `cmd_transition` wrote that trailer only
when `--note` was passed, so a hold released by the two-verb path (`note` then `transition`), or by
a human moving the label in the tracker, left a ticket that had genuinely moved on still reporting
`blocked_report {released: false}` — byte-identical to a block nobody had answered. `references/
triage.md` gave `false` no meaning, so the only repair the surface suggested was "write the missing
label", and a wave re-applied `blocked` to finished work, silently, every wave. Confirmed live
(build-2 #83, released and verified at 05:14, re-blocked at 08:57).

**Shipped:** `cmd_transition` now treats a move off `blocked` as the release itself and stamps the
`orch-unblock` trailer whether or not `--note` was passed — without a note it posts a short
trailer-carrying comment saying the hold ended here. `_release_noted` still gates it, so a re-run
does not double it, and the hold guard now runs before either write, so a release the guard will
refuse leaves nothing behind. `snapshot.jq` `blocked_report_of` takes the ticket's current state
and carries it as `ticket_state`; `references/triage.md` documents the pairing — only
`ticket_state: "blocked"` with `released: false` is an unanswered hold, any other `ticket_state` is
history and never a reason to re-block. Tests: an assertion folded into `07-snapshot.sh`'s
...
**Shipped (P96):** `merge_attempts_of` (in `scripts/lib.jq` since P93 — the citation above predates
that move) now takes the same newest-marker cutoff `rejections_of` uses, over two markers:
`orch-merge-reset`, written by the new `lane.sh merge-reset`, and `orch-scope-reset`, written by
`rescope`. A rescoped ticket therefore sheds its merge history along with its rejections, and a
ticket whose merges failed on something since fixed has a reset that does not touch its scope. The
same rewrite anchors the scan on the trailer's full form, which closes D-SKILL-13. Section 07 gains
three cases (a reset retires older attempts and not newer ones; prose naming the trailer counts
zero; a scope reset retires merge attempts too), section 16 gains six on the verb, and
`scripts/mutate.sh` gains `merge-reset-cutoff` and `merge-attempt-anchor`.
...
**Shipped (P96):** the first of the two fix shapes above — `merge_attempts_of` (in `scripts/lib.jq`
since P93, not `snapshot.jq` as cited) matches `<!-- orch-merge-attempt <n> -->` in full rather than
the bare substring, so a blocked report naming the marker it reports on counts zero. That form also
excludes P62's base-red attempts on its own, since those carry ` base-red=… fix=…` before the `-->`.
The suite gains the fixture this entry says it lacked: a note that merely mentions the marker,
asserted at 2 attempts, not 3.

### D-SNAP-18 · `merge_hold_of` parks a ticket on a trailer quoted in prose
*Closed 2026-08-15.*

`merge_hold_of` derived the base-red park with a `scan()` over the marker's INNER text
(`orch-merge-attempt <n> base-red=<check> fix=<n>`), matching it anywhere in a note body rather than
only inside the `<!-- … -->` trailer `lane.sh merge-failed` writes. P96 had just anchored its
sibling `merge_attempts_of`, so the two halves of one mechanism read the same marker two different
ways. Any note reproducing that text — most plausibly a blocked report explaining why the ticket is
parked — gave it a `merge_hold` naming a check and a fix issue no lane ever recorded, and the wave
skipped it in the merge queue while that issue stayed open.

**Shipped:** the `scan` is anchored on the trailer's full form,
`<!-- orch-merge-attempt \d+ base-red=(\S+) fix=(\d+) -->`. The check id is `\S+` rather than its
character class because the delimiters and the required ` fix=` already bound the field, and
`lane.sh merge-failed` still validates `A-Za-z0-9._:/#-` on the writer side; the looser field is
also what lets `scripts/mutate.sh` substitute the pattern whole (new registry entry
`merge-hold-anchor`, verdict CAUGHT). Block 7a7b of section 07 gains ticket #52, open fix #62 and
one assertion — `snapshot: prose quoting a base-red trailer does not park the ticket` — folded into
