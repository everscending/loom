...
# UI has no missing-runner reduction: without host browser evidence, review is
# rejected before the sandboxed provider and no reusable candidate can exist.
...
if [ "$(cat "$LOOM_HOME/lanes/gate-297.rc" 2>/dev/null)" = 7 ] \
   && [ ! -e "$LOOM_HOME/ui-pregate-attestations/approved/297.json" ] \
   && [ ! -e "$LOOM_HOME/ui-pregate-attestations/candidates/gate-297.json" ]; then
  ok "ui attestation: a missing UI runner rejects before review and never attests"
else
  bad "ui attestation: missing UI runner reached review or became reusable"
fi
