...
# `diagnosis-hold` is the machine-owned round-three boundary. It refuses a
# stale state or stale latest-verdict identity before writing, and on success
# feeds the exact active FAIL comments through blocked-report + transition.
cat > "$T/diagnosis-thread.json" <<'STUB'
[{"body":"Second exact failure.\n\n<!-- orch-verdict FAIL bbbb2222 class=api-contract -->","created_at":"2026-08-10T10:00:00Z"},
 {"body":"First exact failure.\n\n<!-- orch-verdict FAIL aaaa1111 class=acceptance-gap -->","created_at":"2026-08-10T09:00:00Z"}]
STUB
DGEN=$(jq -L "$(dirname "$LANE")" -r 'include "lib"; active_fail_generation(.)' "$T/diagnosis-thread.json")
jq 'del(.[1])' "$T/diagnosis-thread.json" > "$T/diagnosis-thread-count.json"
jq '.[0].body |= sub("class=api-contract"; "class=changed-class")' \
    "$T/diagnosis-thread.json" > "$T/diagnosis-thread-class.json"
jq '.[0].body |= sub("Second exact failure"; "Changed body, same latest SHA and time")' \
    "$T/diagnosis-thread.json" > "$T/diagnosis-thread-body.json"
cat > "$T/glab-diagnosis-stub.sh" <<'STUB'
#!/usr/bin/env bash
for a in "$@"; do case "$a" in body=@*)
    cat "${a#body=@}" >> "${DBODIES:?}"
    if [ -n "${DPOSTS:-}" ]; then
        n=$(cat "$DPOSTS/count" 2>/dev/null || echo 0); n=$((n+1))
        cp "${a#body=@}" "$DPOSTS/$n"; echo "$n" > "$DPOSTS/count"
    fi ;;
esac; done
echo "$*" >> "${DCALLS:?}"
case "$*" in
    *"--method POST"*"issues/8/notes"*) echo '{}' ;;
    *"--method PUT"*"issues/8"*) [ -n "${DRACE_LOG:-}" ] && echo reached > "$DRACE_LOG"; echo '{}' ;;
    *"issues/8/notes"*)
        if [ "${DTRUNCATE:-0}" = 1 ]; then
            n=$(echo "$*" | sed -n 's/.*per_page=\([0-9]*\).*/\1/p')
            jq ".[0:${n:-30}]" "${DTHREAD:?}"
        else cat "${DTHREAD:?}"; fi ;;
    *"issues/8"*)
        printf '{"iid":8,"state":"%s","labels":["build-3","%s"]}\n' "${DISSUESTATE:-opened}" "${DSTATE:-in-progress}" ;;
    *labels*|*milestones*) echo '[]' ;;
    *) echo '{}' ;;
esac
STUB
chmod +x "$T/glab-diagnosis-stub.sh"
DCALLS="$T/diagnosis-calls"; DBODIES="$T/diagnosis-bodies"; DPOSTS="$T/diagnosis-posts"
mkdir -p "$DPOSTS"; : > "$DCALLS"; : > "$DBODIES"
LOOM_HOME="$EVH" GLAB_CMD="$T/glab-diagnosis-stub.sh" \
    DTHREAD="$T/diagnosis-thread.json" DCALLS="$DCALLS" DBODIES="$DBODIES" DPOSTS="$DPOSTS" \
    "$LANE" diagnosis-hold 8 --if-current in-progress \
      --expected-generation "$DGEN" >/dev/null 2>&1
if grep -q "First exact failure" "$DBODIES" \
   && grep -q "Second exact failure" "$DBODIES" \
   && grep -q "HEAD:.*bbbb2222" "$DBODIES" \
   && grep -Fq "orch-diagnosis-source state=in-progress generation=$DGEN" "$DBODIES" \
   && grep -q "orch-blocked category=rejection-cap" "$DBODIES" \
   && grep -q 'issues/8/notes.*per_page=30' "$DCALLS" \
   && [ "$(grep -c -- '--method POST' "$DCALLS")" = 1 ] \
   && ! grep -q -- '--method PUT' "$DCALLS"; then
    ok "diagnosis-hold: one exact guarded FAIL report requires no racy state PUT"
else
    bad "diagnosis-hold: atomic report missing or machine path wrote ticket state"
fi

# A tracker seam ready to interleave another writer immediately before the old
# PUT never fires: the machine protocol has no check-then-unconditional state
# write left to race.
: > "$DCALLS"; : > "$DBODIES"
DRACE_LOG="$T/diagnosis-put-race"; : > "$DRACE_LOG"
LOOM_HOME="$EVH" GLAB_CMD="$T/glab-diagnosis-stub.sh" \
    DTHREAD="$T/diagnosis-thread.json" DCALLS="$DCALLS" DBODIES="$DBODIES" DRACE_LOG="$DRACE_LOG" \
    "$LANE" diagnosis-hold 8 --if-current in-progress \
      --expected-generation "$DGEN" >/dev/null 2>&1 \
    && if [ ! -s "$DRACE_LOG" ] && ! grep -q -- '--method PUT' "$DCALLS"; then
           ok "diagnosis-hold: final-GET/PUT interleaving is eliminated, not guessed safe"
       else
           bad "diagnosis-hold: racy state PUT remains reachable"
       fi \
    || bad "diagnosis-hold: note-owned protocol failed without a state PUT"

# Feed the two bodies written by the real verb back through the public
# snapshot. The report deliberately contains verbatim FAIL comments, including
# their orch-verdict trailers; those copies are evidence, not new verdicts.
DSFX="$T/diagnosis-snapshot-fx"; make_glab_fixture "$DSFX"
jq '(.[] | select(.iid==12) | .labels) |=
      (map(select(. != "blocked" and . != "merge-queue" and . != "review" and
                  . != "in-progress" and . != "ready-for-agent")) + ["in-progress"])' \
    "$DSFX/open.json" > "$T/diagnosis-snapshot-open.json"
jq -n --rawfile report "$DPOSTS/1" \
      --slurpfile fails "$T/diagnosis-thread.json" \
      '[{"system":false,"created_at":"2026-08-10T11:00:00Z","body":$report}] + $fails[0]' \
    > "$DSFX/notes-12.json"
GLAB_CMD="$DSFX/glab-stub.sh" STUB_OPEN="$T/diagnosis-snapshot-open.json" \
    STUB_LOG="$T/diagnosis-snapshot-calls" "$TICK" snapshot \
    > "$T/diagnosis-real-snapshot.json" 2>/dev/null
if jq -e --arg gen "$DGEN" '.tickets[] | select(.id==12)
          | .rejections.total == 2
            and .rejections.generation == $gen
            and .blocked_report.released == false' \
        "$T/diagnosis-real-snapshot.json" >/dev/null; then
    ok "diagnosis-hold→snapshot: copied verdict trailers do not duplicate the frozen generation"
else
    bad "diagnosis-hold→snapshot: report copies became new verdicts or invalidated the hold"
fi

# More than 30 comments can hold older active verdict trailers. The tracker
# enforces the requested cap here: lane validation must use the same newest-30
# generation that snapshot freezes, neither 29 nor 100.
jq -n '([range(0;29) | {created_at:("2026-08-11T12:" + ((59-.)|tostring) + ":00Z"),body:("new comment " + (.|tostring))}]
        + [{created_at:"2026-08-11T11:20:00Z",body:"visible failure\n\n<!-- orch-verdict FAIL eeee5555 class=visible -->"}]
        + [range(0;5) | {created_at:("2026-08-10T10:0" + (.|tostring) + ":00Z"),body:("older comment " + (.|tostring))}]
        + [{created_at:"2026-08-09T09:00:00Z",body:"older active failure\n\n<!-- orch-verdict FAIL ffff6666 class=outside-window -->"}])' \
    > "$T/diagnosis-window-thread.json"
DWINDOW_GEN=$(jq -L "$(dirname "$LANE")" -r 'include "lib"; .[0:30] | active_fail_generation(.)' \
    "$T/diagnosis-window-thread.json")
: > "$DCALLS"; : > "$DBODIES"
if LOOM_HOME="$EVH" GLAB_CMD="$T/glab-diagnosis-stub.sh" \
      DTHREAD="$T/diagnosis-window-thread.json" DTRUNCATE=1 DCALLS="$DCALLS" DBODIES="$DBODIES" \
      "$LANE" diagnosis-hold 8 --if-current in-progress \
        --expected-generation "$DWINDOW_GEN" >/dev/null 2>&1 \
   && grep -q 'issues/8/notes.*per_page=30' "$DCALLS"; then
    ok "diagnosis-hold: newest-30 generation matches snapshot beyond a 30-comment history"
else
    bad "diagnosis-hold: evidence horizon drifted from snapshot"
fi

for stale in state closed count class body; do
    : > "$DCALLS"; : > "$DBODIES"
    dstate=in-progress; dissue=opened; dthread="$T/diagnosis-thread.json"
    case "$stale" in
        state) dstate=review ;;
        closed) dissue=closed ;;
        count) dthread="$T/diagnosis-thread-count.json" ;;
        class) dthread="$T/diagnosis-thread-class.json" ;;
        body) dthread="$T/diagnosis-thread-body.json" ;;
    esac
    LOOM_HOME="$EVH" GLAB_CMD="$T/glab-diagnosis-stub.sh" \
        DTHREAD="$dthread" DCALLS="$DCALLS" DBODIES="$DBODIES" \
        DSTATE="$dstate" DISSUESTATE="$dissue" \
        "$LANE" diagnosis-hold 8 --if-current in-progress \
          --expected-generation "$DGEN" >/dev/null 2>&1 \
        && bad "diagnosis-hold: accepted stale $stale evidence" \
        || if grep -qE -- '--method (POST|PUT)' "$DCALLS"; then
               bad "diagnosis-hold: stale $stale evidence reached a write"
           else
               ok "diagnosis-hold: stale $stale evidence is refused before writes"
           fi
done
