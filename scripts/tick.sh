...
cmd_tick() {
    ...
    # before it took the lock. The `watcher-arm` / `quiet-tick` verbs are kept
    # working for repos still running the old split agent.
...
# --- quiescence watcher daemon ---------------------------------------------
# Deliberately SEPARATE from the build agent: the watcher observes and
# notifies, never schedules and never spends — so it is safe to arm
# automatically on the first tick, including in manual-drive where the human
# does not trust (or want) the scheduling heartbeat. It self-disarms when the
# build completes. (Paid for: build-1 2026-08-02 — quiet-detection tied to
# ticks never fires in the one case that matters, the tick chain dying; and
# the human ruled the scheduling agent out as the watcher's host.)
WATCH_LABEL="$ORCH_LABEL.watch"

_write_watcher_plist() { # <path> <interval>
    local path="$1" interval="$2" b d toolpath=""
    for b in glab jq git bash osascript curl; do
        d="$(command -v "$b" 2>/dev/null)" && toolpath="$toolpath:$(dirname "$d")"
    done
    local pathval="${toolpath#:}:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin"
    cat > "$path" <<EOF
...
<dict>
    ...
    <key>ProgramArguments</key>
    <array><string>$SELF_PATH</string><string>quiet-tick</string></array>
    <key>EnvironmentVariables</key>
    <dict>
        <key>ORCH_REPO</key><string>$REPO_ROOT</string>
        <key>ORCH_HOME</key><string>$ORCH_HOME</string>
    ...
    <key>RunAtLoad</key><true/>
    <key>KeepAlive</key><false/>
    <key>StandardOutPath</key><string>$LOGS_DIR/launchd.watch.out.log</string>
    <key>StandardErrorPath</key><string>$LOGS_DIR/launchd.watch.err.log</string>
</dict>
</plist>
EOF
}

cmd_watcher_arm() { # watcher-arm [interval-seconds]
    local interval="${1:-60}"
    local plist="$PLIST_DIR/$WATCH_LABEL.plist"
    if "$LAUNCHCTL_CMD" print "gui/$(id -u)/$WATCH_LABEL" >/dev/null 2>&1; then
        return 0                                # already armed — idempotent
    fi
    mkdir -p "$LOGS_DIR" "$PLIST_DIR"
    _write_watcher_plist "$plist" "$interval"
    "$LAUNCHCTL_CMD" bootout "gui/$(id -u)/$WATCH_LABEL" 2>/dev/null || true
    "$LAUNCHCTL_CMD" bootstrap "gui/$(id -u)" "$plist"
    echo "orchestrate: quiescence watcher ARMED ($WATCH_LABEL, ${interval}s — observes and notifies only, never spends)"
}
...
cmd_watcher_disarm() {
    ...
    "$LAUNCHCTL_CMD" bootout "gui/$(id -u)/$WATCH_LABEL" 2>/dev/null || true
    rm -f "$plist"
    # A frozen progress stamp with no watcher maintaining it would misread a
    # healthy long-running lane as stale — remove the stamps so lane-status
    # falls back to the stream-mtime clock.
    rm -f "$LANES_DIR"/*.progress "$ORCH_HOME/wave.progress" 2>/dev/null || true
    echo "orchestrate: quiescence watcher disarmed ($WATCH_LABEL)"
}
...
# One watcher firing: stamp progress, flag wedges, classify quiet, notify on
# state change — and when the build is complete take the watcher down with
# it. Read-only against the tracker; touches only stamps and sentinels in
# the run dir.
cmd_quiet_tick() {
    # A watcher whose repo is gone is a leak (deleted worktree, test sandbox,
    # moved checkout): take itself down on the first firing instead of dying
    # every 60s forever. Self-healing for any arm path the seams miss.
    if [ ! -d "$REPO_ROOT" ]; then
        cmd_watcher_disarm >/dev/null 2>&1 || :
        return 0
    fi
    _stamp_progress
    _notify_stale
    ...
    _notify_quiet "$q"
    if [ "$q" = complete ]; then
        ( sleep 2; "$SELF_PATH" watcher-disarm ) >/dev/null 2>&1 &
    fi
    return 0
}
...
case "${1:-}" in
    ...
    watcher-arm) shift; cmd_watcher_arm "$@" ;;
    watcher-disarm) shift; cmd_watcher_disarm "$@" ;;
